...
1. **Fetch PR comments**
   ...
   ```bash
   gh api repos/yannikstauffer/newsflash/pulls/<number>/comments --jq '.[] | {path, line, body, user: .user.login}'
   ```

   And top-level review bodies:

   ```bash
   gh api repos/yannikstauffer/newsflash/pulls/<number>/reviews --jq '.[] | select(.body != "") | {body, state, user: .user.login}'
   ```
